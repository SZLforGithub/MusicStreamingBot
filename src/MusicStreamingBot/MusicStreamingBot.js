    ...
    constructor(token) {
        ...
        client.on('message', (message) => {
            ...
            if (message.content.substring(0, 2) == '%%') {
                ...
                if (cmd[0] == 'url') {
                    this.completeMusicRelayer.processUrl(cmd[1]).then(value => channel.send(value.outputMessage));
                }
                else if (cmd[0] == 'keyword') {
                    this.completeMusicRelayer.keywordToUrl(message.content.substring('%%keyword'.length)).then(value => channel.send(value.outputMessage));
